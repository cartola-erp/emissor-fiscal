...
DROP PROCEDURE IF EXISTS `PR_CADASTRA_TRIBUTACOES_FEDERAIS`;

DELIMITER $$ 

CREATE DEFINER=`root`@`localhost` PROCEDURE `PR_CADASTRA_TRIBUTACOES_FEDERAIS`() 
BEGIN
    ...
    DECLARE CUR_NCMS_NOVEMBRO CURSOR FOR SELECT n.ncm_id FROM ncms n WHERE n.nume IN (SELECT v.ncm FROM NCMS_NOVEMBRO_VENDA v WHERE v.NCM 
		NOT IN(SELECT m.NCM FROM ncms_monofasicos m));

    DECLARE CONTINUE HANDLER FOR NOT FOUND SET terminou = 1;
	
    OPEN CUR_NCMS_NOVEMBRO;
	
	updateValor : LOOP 
		FETCH CUR_NCMS_NOVEMBRO INTO ID_NCM;
		IF terminou = 1 THEN 
			LEAVE updateValor;
		END IF;
		...
		-- FALTA A GABRIELA CONFIRMAR
		INSERT INTO `trib_fede` (cofins_aliq, cofins_base, cofins_cst, finalidade, ipi_aliq, ipi_base, ipi_cst, mens, pis_aliq, pis_base, pis_cst, regime_tributario, oper_id, ncm_id) VALUES (0.076000, 1.000000, 1, 'CONSUMO', 0.000000, 0.000000, 1, 'Tributacao Cadastrada - 29/06 - VIA SCRIPT', 0.016500, 1.000000, 1, 'NORMAL', 8, ID_NCM);
		SELECT "TRIB_FEDE | 8 - DEVOLUCAO DO CLIENTE", ROW_COUNT();
		
		-- FALTA A GABRIELA CONFIRMAR
		INSERT INTO `trib_fede` (cofins_aliq, cofins_base, cofins_cst, finalidade, ipi_aliq, ipi_base, ipi_cst, mens, pis_aliq, pis_base, pis_cst, regime_tributario, oper_id, ncm_id) VALUES (0.076000, 1.000000, 1, 'CONSUMO', 0.000000, 0.000000, 1, 'Tributacao Cadastrada - 29/06 - VIA SCRIPT', 0.016500, 1.000000, 1, 'NORMAL', 9, ID_NCM);
	...
	CLOSE CUR_NCMS_NOVEMBRO;

END $$
DELIMITER ;

CALL PR_CADASTRA_TRIBUTACOES_FEDERAIS();
